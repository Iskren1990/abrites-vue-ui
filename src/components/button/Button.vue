...
import { withDefaults, computed, defineEmits, defineProps, ref } from "vue";
...
function continuousTrigger(e: Event) {
  ...
  continuousDelayTimer.value = window.setTimeout(
    () =>
      (continuousPeriodicTimer.value = window.setInterval(
        () => trigger(e),
        props.continuous
      )),
...
function cancelContinuousTrigger() {
  window.clearTimeout(continuousDelayTimer.value);
